...
const parseDefinitions = (
  ...
  const definition = wikitext
    ...
    .replace(/{{(?:lb|lbl|label)\|[\w-]+\|(.[^{}]+)}}/g, (_, label) =>
    ...
    .replace(
      /{{(?:non[-\s]gloss(?:\sdefinition)?|(?:n-g)|(?:ngd))\|(.[^{}]+)}}/g,
      "$1"
    )
  ...
  if (newLineIndex === -1 || wikitext.charAt(newLineIndex + 1) !== "#") {
    return [...definitions, definition].filter(
      (def) => def.match(/^(#[^\s]+)\s/) == null
    );
  }

  return parseDefinitions(wikitext, newLineIndex + 1, [
    ...definitions,
    definition,
  ]);
};
